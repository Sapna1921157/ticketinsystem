...
const verifyToken = async (req, res, next) => {
  ...
  try {
    ...
    dbToken = dbConn.query(`select * from users where username = '${username}'`, async (err, getresp) => {
     ...
     console.log("err",err)
        ...
        if (token == await resToken) {
          ...
          dbConn.query(`select * from users WHERE updated_at < DATE_SUB( NOW(), INTERVAL 120 MINUTE ) and user_id = '${getresp[0].user_id}' and token != "I am not having token"`,   async (err, newres) => {
       
            if(newres){
              if (newres.length>0){
                dbConn.query(`update users set token='I am not having token' where token = '${token}'`,   async (err, getres) => {
                ...
                dbConn.query(`Update users set updated_at = '${generate_date}' where username = '${username}'`, async (err, updateRes) => {
                  if (updateRes) {
                    dbConn.query(`SELECT *
                                  FROM UserLogs
    ...
    let sql = `update users set token='I am not having token' where token = '${token}'`
